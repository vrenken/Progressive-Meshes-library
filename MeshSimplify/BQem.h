...
namespace hh
{
	...
	public:
		...
		void copy(const base& qem) override
		{
			ASSERTXX(check(qem)), _q = static_cast<const type&>(qem)._q;
		}
		void add(const base& qem) override
		{
			ASSERTXX(check(qem)), _q.add(static_cast<const type&>(qem)._q);
		}
		void scale(float f) override
		{
			_q.scale(f);
		}
		void set_d2_from_plane(const float* dir, float d) override
		{
			_q.set_d2_from_plane(dir, d);
		}
		void set_d2_from_point(const float* p0) override
		{
		...
		float evaluate(const float* p) const override
		{
			return _q.evaluate(p);
		}
		bool compute_minp(float* minp) const override
		{
		...
		bool fast_minp_constr_lf(float* minp, const float* lf) const override
		{
		...
		bool check_type(int ptsize, int pn) const override
		{
	...
	private:
		Qem<T, n> _q;
		bool check(const base& qem)
		{
			return qem.check_type(sizeof(T), n);
		}
