...
// Gather a ring of vertices around vertex v.
	...
	inline Set<fptinfo*>& f_setpts(Face f)
	{
		return func_f_setpts(f).setpts;
	}
#else
	...
	inline Set<eptinfo*>& e_setpts(Edge e)
	{
		return func_e_setpts(e).setpts;
	}
#else
	...
	inline WedgeInfo& c_winfo(Corner c)
	{
		return gwinfo[c_wedge_id(c)];
	}
	...
	enum EResult
	{
		R_success, R_dih, R_illegal, R_NUM
	};
	...
	void init_qem()
	{
		...
		else
		{
			...
			if (1)
			{  // Verify QEM's are initially zero
		...
		for_int(irep, nf)
		{
			...
			{  // two directions (CCW, CLW)
				...
				for (;;)
				{
					...
					assertx((c1 == pc) == (pc1 == c));
					if (c1 == pc)
					{  // two faces adjacent on same vertex
					...
					{  // two faces across from {f1|f2}
					   // if (cnwid >= 0) break;
						...
						if (!same)
						{
							if (1 && c_wedge_id(pc) == c_wedge_id(pc1))
							{  // 20090415
	...
	bool gather_nn(Edge e, NewMeshNei& nn)
	{
		...
		// Gather ar_vdisc, ar_epts, ar_eptv, ar_eptretire.
		{
			...
			for (;;)
			{
				nva++;
				for_int(i, 2)
				{  // run this at most 2 times (usually once)
