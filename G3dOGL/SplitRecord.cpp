...
void SplitRecord::applySplit(SimplicialComplex& K)
{
	...
	while (!pq[dim].empty())
	{
		...
		if (outcome == SplitRecord::V_EDGE)
		{  // create edge between vs and vt
	...
	while (!pq[dim].empty())
	{
		...
		else if (outcome == SplitRecord::E_VSVT || outcome == SplitRecord::E_F)
		{  // create simplex to vt
