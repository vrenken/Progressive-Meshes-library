...
namespace
{
...
#else
	VertexLOD& v_lod(Vertex)
	{
		assertnever("");
	}
	CornerLOD& c_lod(Corner)
	{
		assertnever("");
	}
#endif
	...
	public:
		...
		bool defined(int segn) const
		{
			return !!obp(segn);
		}  // recursion on links not permitted
	...
	Pixel pack_color(const A3dColor& col)
	{
		return Vector4(col[0], col[1], col[2], 1.f).pixel();
	}
	...
	void normalmap_activate()
	{
		assertx(pnormalmap)->activate();
	}

	void normalmap_deactivate()
	{
	...
	void draw_list(CArrayView<unique_ptr<Node>> arn)
	{
		...
		for_int(i, arn.num())
		{  // note: index i is also incremented within loop
			if (ii)
			{
				if (!--ii)
					ii = quicki;
	...
	void draw_mesh(GMesh& mesh)
	{
		...
		if (strip_lines && !lquickmode)
		{
			...
			for (Face f : fa)
			{
				...
				{  // for cover faces in Edge Processing project.
	...
	void draw_all()
	{
		...
		if (!is_init)
		{  // make sure all loaded to avoid later stalls
			is_init = true;
	...
	void process_print()
	{
		...
		{                            // save GL_READ_BUFFER
			if (0)
			{                   // 20160907 disabled because we do want most-recent rendering which is in backbuffer
	...
	void GXobject::morph(float finterp)
	{  // finterp == 1.f is new,   finterp == 0.f is old
		use_dl = false;
		GMesh& mesh = *pmesh;
		mesh_init(mesh);
		bool has_v_color = mesh.gflags().flag(mflag_v_colors);
...
// variables
	...
	void sr_regulator()
	{
		...
		static int count;
		if (++count > 50)
		{  // ignore the transient behavior in the first 50 frames
