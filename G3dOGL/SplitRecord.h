...
namespace hh
{
	...
	public:
		...
		int getNextOutcome()
		{
			return _outcome[_outcome_index++];
		}
		...
		const Point& getDeltap() const
		{
			return _deltap;
		}

		// meaningful after applyGMSplit
		const std::vector<Simplex>& getNewFacets() const
		{
			return new_facets;
		}
		// meaningful after applyCmpSplit
		const std::vector<Simplex>& getNewSimplices() const
		{
