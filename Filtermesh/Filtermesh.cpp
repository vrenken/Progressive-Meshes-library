...
namespace
{
	...
	int nfaces = 0;
	float maxcrit = 1e20f;
	enum class EReduceCriterion
	{
		undefined, length, inscribed, volume, qem
	};
	...
	GMesh split_corners(const GMesh& mo, bool split_matbnd)
	{
		...
		for (Vertex vo : mo.ordered_vertices())
		{
			...
			for (Corner corep : mo.corners(vo))
			{
				...
				Face fo = forep;
				for (;;)
				{  // find f: most_clw, or frep if closed
	...
	void do_trisubdiv()
	{
		...
		for (Vertex v : arv)
		{
			...
			else if (nesharp == 2 && ne == 2)
			{  // boundary corner: unmoved
				continue;
			}
			else if (nesharp == 2)
			{  // crease: cubic bspline boundary
				h = hsharp + 6.f * Homogeneous(mesh.point(v));
			}
			else
			{  // interior: quartic bspline interior
		...
		for_map_key_value(mapvp, [&](Vertex v, const Point& p)
			{
				mesh.set_point(v, p);
			});
	...
	void do_desbrunsmooth(Args& args)
	{
		...
		for (Vertex v : mesh.vertices())
		{
			...
			assertx(sum_w);
			if (0)
			{  // this makes lambda scale-dependent (bad)
				for_int(j, nei_vi.num()) nei_w[j] /= (4.f * sum_area);
				sum_w /= (4.f * sum_area);
			}
			else
			{  // "normalized" version from Section 5.5
	...
	enum class ETriType
	{
		dshort, dlong, alternating, xuvdiag, duvdiag, odddiag
	};
	...
	void triangulate_quads(ETriType type)
	{
		...
		while (!stackf.empty())
		{
			...
			switch (type)
			{
			case ETriType::dshort:
			{  // Use GIM rule to select shorter diagonal.
			...
			case ETriType::alternating:
			{  // Use alternating diagonals
			...
			case ETriType::xuvdiag:
			{  // Use X shaped diagonal pattern on [0..1][0..1] domain
				float maxv = -BIGFLOAT;
				int maxi = -1;
				for_int(i, 4)
				{
			...
			case ETriType::duvdiag:
			{  // Use diamond shaped diagonal pattern on [0..1][0..1] domain
				float maxv = -BIGFLOAT;
				int maxi = -1;
				for_int(i, 4)
				{
			...
			case ETriType::odddiag:
			{  // Create odd-valence vertices
				int fid = mesh.face_id(f) - 1;
				int iy = fid / sqrt_nfaces;
				int ix = fid - iy * sqrt_nfaces;
	...
	void do_lengthc()
	{
		reducecrit = EReduceCriterion::length;
	}

	void do_inscribedc()
	{
		reducecrit = EReduceCriterion::inscribed;
	}

	void do_volumec()
	{
		reducecrit = EReduceCriterion::volume;
	}

	void do_qemc()
	{
		reducecrit = EReduceCriterion::qem;
	}
 ...
 // == dot(poly.get_normal(), p-clp)
	...
	Point compute_hull_point(Vertex v, float offset)
	{
		...
		int n_m1 = int(count_if(ar_lf, [](const LinearFunc& lf)
			{
				return lf.offset > 0.f;
			}));
...
//					}
	...
	void do_trimpts(Args& args)
	{
		...
		if (getenv_bool("CIRCUMRADIUS"))
		{  // not really the circumradius!
