...
namespace hh
{
	...
	public:
		...
		{
			return hedge_from_ef(e, f)->_prev->_edge;
		}
		Edge ccw_edge(Face f, Edge e) const
		{
			return hedge_from_ef(e, f)->_next->_edge;
		}
		Vertex clw_vertex(Face f, Vertex v) const
		{
			return get_hedge(v, f)->_prev->_vert;
		}  // slow
		Vertex ccw_vertex(Face f, Vertex v) const
		{
			return get_hedge(v, f)->_next->_vert;
		}  // slow
		Edge clw_edge(Face f, Vertex v) const
		{
			return get_hedge(v, f)->_edge;
		}             // slow
		Edge ccw_edge(Face f, Vertex v) const
		{
			return get_hedge(v, f)->_next->_edge;
		}      // slow
...
// ** Edge:
		...
		{
			return herep(e)->_prev->_vert;
		}
		Vertex vertex2(Edge e) const
		{
			return herep(e)->_vert;
		}
		...
		{
			return (ASSERTX(i == 0 || i == 1), i == 0 ? face1(e) : face2(e));
		}
		...
		Edge clw_boundary(Edge e) const
		{
			return opp_boundary(e, vertex2(e));
		}  // is_boundary(e)
		Edge ccw_boundary(Edge e) const
		{
			return opp_boundary(e, vertex1(e));
		}  // is_boundary(e)
		...
		{
			return c->_sym ? c->_sym->_prev : nullptr;
		}  // around vertex
...
// ** Counting routines (fast):
		...
		const Flags& gflags() const
		{
			return _flags;
		}
		Flags& flags(Vertex v)
		{
			return v->_flags;
		}
		const Flags& flags(Vertex v) const
		{
			return v->_flags;
		}
		Flags& flags(Face f)
		...
		const Flags& flags(Face f) const
		{
			return f->_flags;
		}
		Flags& flags(Edge e)
		...
		const Flags& flags(Edge e) const
		{
			return e->_flags;
		}
		...
		{
			return _id2vertex.values();
		}
		Faces_range faces() const
		{
			return _id2face.values();
		}
		...
		OrderedVertices_range ordered_vertices() const
		{
			return OrderedVertices_range(*this);
		}
		OrderedFaces_range ordered_faces() const
		{
			return OrderedFaces_range(*this);
		}
		...
		WV_range ccw_vertices(Vertex v) const
		{
			return WV_range(*this, v);
		}
		WF_range ccw_faces(Vertex v) const
		{
			return WF_range(*this, v);
		}
		WE_range ccw_edges(Vertex v) const
		{
			return WE_range(*this, v);
		}
		WC_range ccw_corners(Vertex v) const
		{
			return WC_range(*this, v);
		}
	...
	private:
		...
		public:
			...
			bool operator!=(const Edges_iterator& rhs) const
			{
				return _hcur != rhs._hcur || _vcur != rhs._vcur;
			}
			Edge operator*() const
			{
				return (ASSERTX(_hcur != _hend), (*_hcur)->_edge);
			}
		...
		public:
			...
			bool operator!=(const VV_iterator& rhs) const
			{
				ASSERTX(!rhs._extrav);
				return _extrav || _it != rhs._it;
			}
			Vertex operator*() const
			{
				return _extrav ? _extrav : (*_it)->_vert;
			}
		...
		public:
			...
			bool operator!=(const VF_iterator& rhs) const
			{
				return _it != rhs._it;
			}
			Face operator*() const
		...
		struct VF_range
		{
			...
			VF_iterator begin() const
			{
				return VF_iterator(_ar.begin());
			}
			VF_iterator end() const
			{
				return VF_iterator(_ar.end());
			}
			CArrayView<HEdge> _ar;
		};
		class VE_iterator
		...
		public:
			...
			{
				ASSERTX(!rhs._extrae);
				return _extrae || _it != rhs._it;
			}
			Edge operator*() const
			{
				return _extrae ? _extrae : (*_it)->_edge;
			}
		...
		struct VE_range
		{
			...
			VE_iterator begin() const
			{
				return VE_iterator(_ar.begin());
			}
			VE_iterator end() const
			{
				return VE_iterator(_ar.end());
			}
			CArrayView<HEdge> _ar;
		};
		class VC_iterator
		...
		public:
			...
			bool operator!=(const VC_iterator& rhs) const
			{
				return _it != rhs._it;
			}
		...
		struct VC_range
		{
			...
			VC_iterator begin() const
			{
				return VC_iterator(_ar.begin());
			}
			VC_iterator end() const
			{
				return VC_iterator(_ar.end());
			}
			CArrayView<HEdge> _ar;
		};
		// Face Iter
		...
		struct FV_range
		{
			...
			FV_iterator begin() const
			{
				return FV_iterator(_herep, true);
			}
			FV_iterator end() const
			{
				return FV_iterator(_herep, false);
			}
			HEdge _herep;
		};
		class FF_iterator
		...
		public:
			...
			bool operator!=(const FF_iterator& rhs) const
			{
				ASSERTX(!rhs._beg);
				return _beg || _it != rhs._it;
			}
		...
		struct FF_range
		{
			...
			FF_iterator begin() const
			{
				return FF_iterator(_herep, true);
			}
			FF_iterator end() const
			{
				return FF_iterator(_herep, false);
			}
			HEdge _herep;
		};
		class FE_iterator
		...
		public:
			...
			bool operator!=(const FE_iterator& rhs) const
			{
				ASSERTX(!rhs._beg);
				return _beg || _it != rhs._it;
			}
		...
		struct FE_range
		{
			...
			FE_iterator begin() const
			{
				return FE_iterator(_herep, true);
			}
			FE_iterator end() const
			{
				return FE_iterator(_herep, false);
			}
			HEdge _herep;
		};
		class FC_iterator
		...
		public:
			...
			bool operator!=(const FC_iterator& rhs) const
			{
				ASSERTX(!rhs._beg);
				return _beg || _it != rhs._it;
			}
		...
		struct FC_range
		{
			...
			FC_iterator begin() const
			{
				return FC_iterator(_herep, true);
			}
			FC_iterator end() const
			{
				return FC_iterator(_herep, false);
			}
			HEdge _herep;
		};
		// Edge Iter
		struct EV_range : Vec2<Vertex>
		{  // always 2 vertices
	...
	private:
		...
		{
			return he->_next->_sym;
		}  // may return nullptr
		HEdge ccw_hedge(HEdge he) const
		{
			return he->_sym ? he->_sym->_prev : nullptr;
		}
		HEdge herep(Vertex v) const
		{
			return !v->_arhe.num() ? nullptr : v->_arhe[0]->_prev;
		}
