...
namespace hh
{
	...
	public:  // discouraged
		Vec4<float>& access_private()
		{
			return _c;
		}
		const Vec4<float>& access_private() const
		{
			return _c;
		}
	...
	private:
		...
		friend float mag2(const Quaternion& q)
		{
			return static_cast<float>(mag2(q._c));
		}
		friend float mag(const Quaternion& q)
		{
			return static_cast<float>(mag(q._c));
		}
		friend float dot(const Quaternion& q1, const Quaternion& q2)
		{
			return static_cast<float>(dot(q1._c, q2._c));
		}
		void normalize()
		{
			_c *= 1.f / assertx(static_cast<float>(mag(_c)));
		}
	};
