...
namespace hh
{
	...
	public:
		...
		explicit Vector4(const Pixel& pix)
		{
			*this = to_Vector4_norm(pix.data());
		}
		explicit Vector4(const Vec4<float>& a)
		{
			load_unaligned(a.data());
		}
		float& operator[](int i)
		{
			return (HH_CHECK_BOUNDS(i, 4), _c[i]);
		}
		...
		friend float mag2(const Vector4& v)
		{
			return dot(v, v);
		}
		friend float dist2(const Vector4& l, const Vector4& r)
		{
			return mag2(l - r);
		}
		friend float sum(const Vector4& v)
		{
			return dot(v, Vector4(1.f));
		}
		friend Vector4 operator-(const Vector4& l)
		{
			return Vector4(0.f) - l;
		}
		...
		{
			return aligned_malloc(s, alignof(type));
		}
		...
		{
			_r = _mm_set_ps(w, z, y, x);
		}  // note reverse ordering
		void load_unaligned(const float* pSrc)
		{
			_r = _mm_loadu_ps(pSrc);
		}
		void store_unaligned(float* pDst) const
		{
			_mm_storeu_ps(pDst, _r);
		}
		void load_aligned(const float* pSrc)
		{
			_r = _mm_load_ps(pSrc);
		}   // VT: vld1q_f32_ex(pSrc, 128);
		void store_aligned(float* pDst) const
		{
			_mm_store_ps(pDst, _r);
		}  // VT: vst1q_f32_ex(pDst, _r, 128);
		friend Vector4 operator+(const Vector4& l, const Vector4& r)
		{
			return _mm_add_ps(l._r, r._r);
		}
		friend Vector4 operator-(const Vector4& l, const Vector4& r)
		{
			return _mm_sub_ps(l._r, r._r);
		}
		friend Vector4 operator*(const Vector4& l, const Vector4& r)
		{
			return _mm_mul_ps(l._r, r._r);
		}
		friend Vector4 operator/(const Vector4& l, const Vector4& r)
		{
			return _mm_div_ps(l._r, r._r);
		}
		friend Vector4 operator+(const Vector4& v, float f)
		{
			return _mm_add_ps(v._r, _mm_set_ps1(f));
		}
		friend Vector4 operator-(const Vector4& v, float f)
		{
			return _mm_sub_ps(v._r, _mm_set_ps1(f));
		}
		friend Vector4 operator*(const Vector4& v, float f)
		{
			return _mm_mul_ps(v._r, _mm_set_ps1(f));
		}
		friend Vector4 operator/(const Vector4& v, float f)
		{
			return _mm_div_ps(v._r, _mm_set_ps1(f));
		}
		...
		{  // For use in RangeOp.h mag(), rms(), dist()
...
// http://stackoverflow.com/questions/3361132/flipping-sign-on-packed-sse-floats
	...
	private:
		...
		friend Vector4 operator+(const Vector4& l, const Vector4& r)
		{
			return vaddq_f32(l._r, r._r);
		}
		friend Vector4 operator-(const Vector4& l, const Vector4& r)
		{
			return vsubq_f32(l._r, r._r);
		}
		friend Vector4 operator*(const Vector4& l, const Vector4& r)
		{
			return vmulq_f32(l._r, r._r);
		}
		friend Vector4 operator/(const Vector4& l, const Vector4& r)
		{
		...
		friend Vector4 operator+(const Vector4& v, float f)
		{
			return vaddq_f32(v._r, vdupq_n_f32(f));
		}
		friend Vector4 operator-(const Vector4& v, float f)
		{
			return vsubq_f32(v._r, vdupq_n_f32(f));
		}
		friend Vector4 operator*(const Vector4& v, float f)
		{
			return vmulq_n_f32(v._r, f);
		}
		friend Vector4 operator/(const Vector4& v, float f)
		{
			return vmulq_n_f32(v._r, 1.f / f);
		}
		Vector4& operator=(const Vector4& r)
		{
			_r = r._r;
		...
		friend Vector4 min(const Vector4& l, const Vector4& r)
		{
			return vminq_f32(l._r, r._r);
		}  // component-wise min
		friend Vector4 max(const Vector4& l, const Vector4& r)
		{
			return vmaxq_f32(l._r, r._r);
		}  // component-wise max
		friend float dot(const Vector4& v1, const Vector4& v2)
		{
			Vector4 v = v1 * v2;
...
//  but unclear.
	...
	inline Vector4 to_Vector4_norm(const uint8_t p[4])
	{
		return to_Vector4_raw(p) * (1.f / 255.f);
	}
	inline Vector4 to_Vector4_raw(const Pixel& pixel)
	{
		return to_Vector4_raw(pixel.data());
	}
	...
	inline Vector4& operator+=(Vector4& l, const Vector4& r)
	{
		return l = l + r;
	}
	inline Vector4& operator-=(Vector4& l, const Vector4& r)
	...
	inline Vector4& operator+=(Vector4& l, float f)
	{
		return l = l + f;
	}
	inline Vector4& operator-=(Vector4& l, float f)
	...
	template <> inline void my_zero(Vector4& v)
	{
		v = Vector4(0.f);
	}
	inline Vector4 interp(const Vector4& v1, const Vector4& v2, float f1 = .5f)
	{
		return f1 * v1 + (1.f - f1) * v2;
	}
