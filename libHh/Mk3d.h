...
namespace hh
{
	...
	public:
		...
		void push()
		{
			_stack_frame.push(_ctm), _stack_framei.push(_ctmi);
		}
		void pop()
		{
			_ctm = _stack_frame.pop(), _ctmi = _stack_framei.pop();
		}
		template <typename Func> void save(Func func)
		{
			push(), func(), pop();
		}
		...
		{
			apply(Frame::rotation(int(axis), angle));
		}
		...
		{
			_ctm = t * _ctm, _ctmi = inverse(_ctm);
		}
		...
		void push_color()
		{
			_stack_color.push(_cc);
		}
		void pop_color()
		{
			assertx(!_stack_color.empty()), _cc = _stack_color.pop();
		}
		template <typename Func> void save_color(Func func)
		{
			push_color(), func(), pop_color();
		}
		void diffuse(float r, float g, float b)
		{
			_cc.d = A3dColor(r, g, b);
		}
		...
		{
			_el.push(A3dVertex(transform(p), Vector(0.f, 0.f, 0.f), _cc));
		}
		...
		void begin_force_polyline(bool b)
		{
			_stack_force_polyline.push(_force_polyline), _force_polyline = b;
		}
		void end_force_polyline()
		{
			_force_polyline = _stack_force_polyline.pop();
		}
		void begin_force_flip(bool b)
		{
			_stack_force_flip.push(_force_flip), _force_flip = b;
		}
		void end_force_flip()
		{
			_force_flip = _stack_force_flip.pop();
		}
		void end_polygon();
	...
	struct MkSave
	{  // save and restore the current geometric transform
		MkSave(Mk3d& mk) : _mk(mk)
		{
			_mk.push();
		}
		~MkSave()
	...
	struct MkSaveColor
	{  // save and restore the current color transform
		MkSaveColor(Mk3d& mk) : _mk(mk)
		{
			_mk.push_color();
		}
		~MkSaveColor()
