...
namespace hh
{
	...
	SubMesh::SubMesh(GMesh& pmesh) : _omesh(pmesh)
	{
		...
		{
			bool is_new;
			Array<Face>& ar = _mofif.enter(f, Array<Face>(), is_new);
			assertx(is_new);
	...
	void SubMesh::subdivide_n(int nsubdiv, int limit, float cosang, bool triang)
	{
		...
		{
			for_int(i, nsubdiv) subdivide_aux(cosang, nullptr);
			if (limit)
			{
				Mvcvh mconv;
	...
	void SubMesh::subdivide_aux(float cosang, Mvcvh* pmconv)
	{
		...
		if (!pmconv)
			convolve_self(mconv2);
		else
			pmconv->compose(mconv2);
		_selrefine = false;
	}
	...
	void SubMesh::refine(Mvcvh& mconv)
	{
		...
		for (Edge e : are)
		{  // was ForStack which went in reverse order
		...
		if (_isquad)
		{
			...
			{  // was ForStack which went in reverse order
				...
				assertx(va.num() == 4);
				Vec4<Vertex> vs;
				for_int(i, 4) vs[i] = menewv.get(_m.edge(va[i], va[(i + 1) % 4]));
	...
	void SubMesh::selectively_refine(Mvcvh& mconv, float cosang)
	{
		...
		struct Svv
		{
			Svv(Vertex v1, Vertex v2) : _v1(v1), _v2(v2)
			{
				if (_v2 < _v1) std::swap(_v1, _v2);
			}
			Vertex _v1, _v2;
		};
		struct hash_Svv
		...
		struct equal_Svv
		{  // : std::equal_to<Svv>
			bool operator()(const Svv& s1, const Svv& s2) const
			{
				return s1._v1 == s2._v1 && s1._v2 == s2._v2;
			}
		};
		hash_Svv hashf(&_m);
	...
	namespace
	{
		...
		constexpr auto k_table_cone_limit = V(0.f, 0.f, 0.f, .5f, .6f, .684624f, .75f, .799356f, .836637f, .865074f, .887058f);
	...
	void SubMesh::averaging_mask(Vertex v, Combvh& comb) const
	{
		...
		if (_m.flags(v).flag(GMesh::vflag_cusp))
		{  // cusp vertex
