...
namespace hh
{
	...
	namespace
	{
		...
		inline bool convert_bool(const string& s)
		{
			return s == "1" || s == "true";
		}
		inline char convert_char(const string& s)
		{
			return s[0];
		}
		inline int convert_int(const string& s)
		{
			return atoi(s.c_str());
		}             // or stoi(s) or to_int(s.c_str())
		inline float convert_float(const string& s)
		{
		...
		{
			return atof(s.c_str());
		}  // or stod(s) or strtod(s.c_str(), nullptr)

		inline string show_bool(bool b)
		{
			return b ? "true" : "false";
		}
	...
	bool Args::check_bool(const string& s)
	{
		return s == "0" || s == "1" || s == "true" || s == "false";
	}

	bool Args::check_char(const string& s)
	{
		return s.size() == 1;
	}
	...
	bool Args::check_double(const string& s)
	{
	...
	void ParseArgs::c(string str, string doc)
	{
		iadd(option(std::move(str), 0, nullptr, nullptr, std::move(doc)));
	}
	...
	{
		if (1 && o.doc != "")
		{  // enforce my "conventions" to properly distinguish flags and parameters
			if (o.narg > 0)
			{
