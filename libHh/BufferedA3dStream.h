...
namespace hh
{
	...
	class RBufferedA3dStream : public RA3dStream
	{  // Read from RBuffer
	public:
		explicit RBufferedA3dStream(RBuffer& b) : _buf(b)
		{
		}
		enum class ERecognize
		{
			parse_error, no, partial, yes
		};
	...
	class WBufferedA3dStream : public WA3dStream
	{  // Write to WBuffer
	public:
		explicit WBufferedA3dStream(WBuffer& b) : _buf(b)
		{
		}
		~WBufferedA3dStream() override
		{
			flush();
		}
		void flush() override
