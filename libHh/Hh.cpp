...
namespace hh
{
	...
	namespace details
	{
		...
		string extract_function_type_name(string s)
		{
			...
			s = std::regex_replace(s, std::regex("std::_[A-Z_][A-Za-z0-9_]*::"), "std::");
			if (remove_at_beginning(s, "hh::details::TypeNameAux<"))
			{  // VC
			...
			else if (remove_at_beginning(s, "static std::string hh::details::TypeNameAux<T>::name() [with T = "))
			{  // GNUC
			...
			else if (remove_at_beginning(s, "static string hh::details::TypeNameAux<T>::name() [with T = "))
			{  // Google opt
				auto i = s.find("; ");
				if (i == string::npos)
				{
				...
				remove_at_beginning(s, "static string hh::details::TypeNameAux<"))
			{  // clang
	...
	namespace
	{
		...
		public:
			using Function = void (*)();
			static void register_function(Function function)
			{
				instance()._functions.push_back(function);
			}
			static void flush()
			{
		...
		{
		public:
			static int increment_count(const char* s)
			{
				return ++instance()._map[s];
			}
			static void flush()
			{
		...
		private:
			...
			void flush_internal()
			{
				...
				struct string_less
				{  // lexicographic comparison; deterministic, unlike pointer comparison
	...
	{
		CleanUp::register_function(function);
	}
...
#pragma clang diagnostic pop
	...
	static void determine_stdout_stderr_needs(bool& pneed_cout, bool& pneed_cerr)
	{
		...
		else if (!isafile(1) && !isafile(2))
		{  // 20170222 for blaze run
			need_cout = false;
		}
		else
		{
