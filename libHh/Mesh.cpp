...
namespace hh
{
	...
	*/
	...
	Mesh::OrderedVertices_range::OrderedVertices_range(const Mesh& mesh)
	{
		...
		sort(_vertices, [&mesh](Vertex v1, Vertex v2)
			{
				return mesh.vertex_id(v1) < mesh.vertex_id(v2);
			});
	}
	...
	Mesh::OrderedFaces_range::OrderedFaces_range(const Mesh& mesh)
	{
		...
		sort(_faces, [&mesh](Face f1, Face f2)
			{
				return mesh.face_id(f1) < mesh.face_id(f2);
			});
	}
