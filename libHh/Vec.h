...
namespace hh
{
	...
	public:
		...
		T& operator[](int i)
		{
			return (HH_CHECK_BOUNDS(i, n), base::operator[](i));
		}
		...
		void assign(CArrayView<T> ar)
		{
			assign_i(ar);
		}
		constexpr type rev() const
		{
			return rev_aux(std::make_index_sequence<n>());
		}
		bool in_range(const type& dims) const
		{
		...
		{
			return CArrayView<T>(a(), n);
		}
		template <int s> Vec<T, s>& head()
		{
			return segment<s>(0);
		}  // V(1, 2, 3).head<2>() == V(1, 2)
		template <int s> const Vec<T, s>& head() const
		{
			return segment<s>(0);
		}
		ArrayView<T> head(int s)
		{
			return segment(0, s);
		}
		CArrayView<T> head(int s) const
		{
			return segment(0, s);
		}
		template <int s> Vec<T, s>& tail()
		{
			return segment<s>(n - s);
		}  // V(1, 2, 3).tail<2>() == V(2, 3)
		template <int s> const Vec<T, s>& tail() const
		{
			return segment<s>(n - s);
		}
		ArrayView<T> tail(int s)
		{
			return segment(n - s, s);
		}
		CArrayView<T> tail(int s) const
		{
			return segment(n - s, s);
		}
		template <int i, int s> Vec<T, s>& segment()
		...
		template <int s> Vec<T, s>& segment(int i)
		{  // V(1, 2, 3, 4).segment<2>(1) == V(2, 3)
		...
		ArrayView<T> segment(int i, int s)
		{
			return (ASSERTXX(check(i, s)), ArrayView<T>(a() + i, s));
		}
		CArrayView<T> segment(int i, int s) const
		{
			return (ASSERTXX(check(i, s)), CArrayView<T>(a() + i, s));
		}
		ArrayView<T> slice(int ib, int ie)
		{
			return segment(ib, ie - ib);
		}
		CArrayView<T> slice(int ib, int ie) const
		{
			return segment(ib, ie - ib);
		}
		...
		static constexpr type all(const T& e)
		{
	...
	template <typename T> constexpr Vec<T, 0> V()
	{
		return Vec<T, 0>();
	}

	// Construct an Vec with two identical elements, e.g. twice(v) == V(v, v).
	template <typename T> constexpr Vec2<T> twice(const T& v)
	{
		return { v, v };
	}

	// Construct an Vec with three identical elements, e.g. thrice(v) == V(v, v, v).
	template <typename T> constexpr Vec3<T> thrice(const T& v)
	{
		return { v, v, v };
	}

	// Construct an Vec with identical elements, e.g. ntimes<4>(.5f) == V(.5f, .5f, .5f, .5f).
	template <int n, typename T> constexpr Vec<T, n> ntimes(const T& v)
	{
	...
	template <typename T, int n1> constexpr Vec<T, n1> concat(const Vec<T, n1>& a1)
	{
		return a1;
	}
	...
	namespace details
	{
		...
		template <typename T, int n> struct Vec_base
		{  // allocates a member variable only if n > 0.
			Vec_base() = default;
		...
		template <typename T> struct Vec_base<T, 0>
		{
			...
			T& operator[](int)
			{
				return *implicit_cast<T*>(nullptr);
			}
	...
	namespace details
	{
		...
		public:
			...
			const Vec<int, D>& operator*() const
			{
				return (ASSERTX(_u[0] < _uU[0]), _u);
			}
			type& operator++()
			{
				static_assert(D > 0, "");
				...
				{  // else VC12 does not unroll this tiny loop
		...
		public:
			...
			Vec_iterator<D> begin() const
			{
				return Vec_iterator<D>(ntimes<D>(0), _uU);
			}
			Vec_iterator<D> end() const
			{
				return Vec_iterator<D>(_uU, _uU);
			}
		...
		public:
			...
			VecL_iterator<D> begin() const
			{
				return VecL_iterator<D>(_uL, _uU);
			}
			VecL_iterator<D> end() const
			{
				return VecL_iterator<D>(_uU, _uU);
			}
	...
	template <int D> details::Vec_range<D> coords(const Vec<int, D>& uU)
	{
		return range(uU);
	}
	// Backwards compatibility; deprecated.
	template <int D> details::VecL_range<D> coordsL(const Vec<int, D>& uL, const Vec<int, D>& uU)
	{
		return range(uL, uU);
	}
...
// clang-format off
	...
	TT G operator+(const G& g1, const G& g2)
	{
		G g; F{ g[i] = g1[i] + g2[i]; } return g;
	}
	TT G operator-(const G& g1, const G& g2)
	{
		G g; F{ g[i] = g1[i] - g2[i]; } return g;
	...
	TT G operator+(const G& g1, T v)
	{
		G g; F{ g[i] = g1[i] + v; } return g;
	}
	TT G operator-(const G& g1, T v)
	{
		G g; F{ g[i] = g1[i] - v; } return g;
	...
	TT G operator+(T v, const G& g1)
	{
		G g; F{ g[i] = v + g1[i]; } return g;
	}
	TT G operator-(T v, const G& g1)
	{
		G g; F{ g[i] = v - g1[i]; } return g;
	...
	TT G& operator+=(G& g1, const G& g2)
	{
		F{ g1[i] += g2[i]; } return g1;
	}
	TT G& operator-=(G& g1, const G& g2)
	{
		F{ g1[i] -= g2[i]; } return g1;
	...
	TT G operator-(const G& g1)
	{
		G g; F{ g[i] = -g1[i]; } return g;
	}

	TT G min(const G& g1, const G& g2)
	{
		G g; F{ g[i] = min(g1[i], g2[i]); } return g;
	}
	TT G max(const G& g1, const G& g2)
	{
		G g; F{ g[i] = max(g1[i], g2[i]); } return g;
	}
