...
namespace
{
	...
	public:
		...
		const WedgeInfo& get(int wid) const
		{
			return _map.get(wid);
		}
	...
	{
		assertx(!_map.num());
		for_map_key_value(mwi._map, [&](int wid, const WedgeInfo& wi)
			{
				set(wid, wi);
			});
	}
	...
	{
	public:
		int existing_id(int vid) const
		{
			return _is_active ? _mapvidvid.get(vid) : vid;
		}
		void activate();
	...
	{
		pfi_prog = make_unique<RFile>(args.get_filename());
	}
	...
	int retrieve_wid(Corner c)
	{
		return !c ? 0 : assertx(c_cwedge_id(c));
	}
